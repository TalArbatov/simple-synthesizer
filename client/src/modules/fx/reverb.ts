import type { EffectUnit, ReverbParams } from '../types.js';

export function createReverb(audioCtx: AudioContext): EffectUnit<ReverbParams> {
  ...
  function generateIR(duration: number): void {
    const sampleRate = audioCtx.sampleRate;
    const length = Math.max(sampleRate * duration, sampleRate * 0.1) | 0;
