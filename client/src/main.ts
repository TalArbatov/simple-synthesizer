...
const adsrGraphs = [
  createADSRGraph(
    document.getElementById('adsr1-canvas') as HTMLCanvasElement,
  ...
  createADSRGraph(
    document.getElementById('adsr2-canvas') as HTMLCanvasElement,
...
const keyboard = createPianoKeyboard(
  document.getElementById('keyboard') as HTMLCanvasElement,
  {
    onNoteOn: freq => {
      engine.noteOn(freq);
...
const eqGraph = createEQGraph(
  document.getElementById('fx-eq-canvas') as HTMLCanvasElement,
  fxChain.eq
);
...
import('./modules/filter-graph.js').then(({ createFilterGraph }) => {
  drawList.push(createFilterGraph(
    document.getElementById('filter1-canvas') as HTMLCanvasElement,
    document.getElementById('filter1-values')!,
    engine.voices[0],
    () => { sync.send({ t: 'filter', n: 0, cutoff: engine.voices[0].cutoff, q: engine.voices[0].resonance }); }
  ));
  drawList.push(createFilterGraph(
    document.getElementById('filter2-canvas') as HTMLCanvasElement,
